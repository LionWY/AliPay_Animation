...
- (void)loadModelWithIndex:(NSInteger)aIndex withTitle:(NSString *)aTitle withIsMore:(BOOL)aMore
{
    Model *model = [[Model alloc] initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT) withTag:aIndex withIsPlus:!self.hasMore withIsMore:aMore];
...
- (void)reloadBtnArrWithArr:(NSArray *)aArr
{
    ...
    if (aArr && [aArr count] >= 1) {
        Model *model = (Model *)[self viewWithTag:100 + [_currentArr count] - [aArr count]];
        
        model.tag = 100 +  [_currentArr count];
        [model resetModelFrame];
        
        for (NSInteger i = [_currentArr count] + 100 - [aArr count]; i < [_currentArr count] + 100; i ++) {
            
            [self loadModelWithIndex:i withTitle:[_currentArr objectAtIndex:i - 100] withIsMore:NO];
        ...
        for (NSInteger j = 0; j < 4; j ++) {
            ...
            if (index < [_currentArr count] + 1) {
                ...
                if (index == [_currentArr count] && self.hasMore) {
                    ...
                    [self loadModelWithIndex:100 + index withTitle:@"More" withIsMore:YES];
                }
                else
                {
